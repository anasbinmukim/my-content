...
function title_backgorund_mobile_image(){
	global $post;	
	$c_pageID = '';	
	if(isset($post)) {	
		$c_pageID = $post->ID;	
	...
	$image_bg_url = '';
	$$image_bg = '';
	//if(get_post_meta($c_pageID, '_rmt_page_bg', true) && rm_is_mobile()){
	if(get_post_meta($c_pageID, '_rmt_page_bg', true)){
		$image_bg_url = get_post_meta($c_pageID, '_rmt_page_bg', true);
		$image_bg = '<style type="text/css">#fullwidth_content_wrap{ background:none; }</style>';
		$image_bg .= '<img class="mobile_bg" src="'.$image_bg_url.'" alt="" />';
	}
	
	return $image_bg;
}
add_shortcode('section-background', 'title_backgorund_mobile_image');