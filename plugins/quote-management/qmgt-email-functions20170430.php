...
if(isset($_POST['generate_email_manually_submit'])){
	if(isset($_POST['generate_email_date'])){
		...
		$email_post_id = generate_daily_email($generate_email_date, $email_quote_id);
	}else{
...
function generate_daily_email($email_date = '', $quote_id = ''){
...
function qmgt_emails_published_action( $ID, $post ){
	...
	add_post_meta($quote_id, '_rmt_quote_email', 'yes');
...
function rm_admin_notification($email_post_id){
	...
	$email_quote_author = '&ndash; '. get_post_meta($email_post_id, '_rmt_quote_author', true);
