                                   ...
                                   // echo  $value_m->ID;echo "<br>";
									...
									// echo "<br>";
										...
										elseif($count==$count_lineitem)
										{
											...
											if (strpos($lineitem['desc'], 'prorated moving') !== false || strpos($lineitem['desc'], 'Month Rent') !== false || strpos($lineitem['desc'], 'Recurrent Rent') !== false) {
												$multisuite_alldata_chunk[$last_suite_id]['rent']=$multisuite_alldata_chunk[$last_suite_id]['rent']+$l_value['total'];
											}
											else{
														if (strpos($l_value['desc'], 'Security Deposit') == false && strpos($l_value['desc'], 'Security Deposit') !== 0 && strpos($l_value['desc'], 'Invoice for') == false && strpos($l_value['desc'], 'Standard Credit') == false && strpos($l_value['desc'], 'Rebill') == false) {
...
// 									echo "</pre>";
								...
								if($yl_suite_number == -1 || $yl_suite_number == 'Y-Membership')
								{
									...
									if (in_array($client_id, $y_membership_clients)) {
										# code...
													if (strpos($l_value['desc'], 'Monthly Rent') !== false || strpos($l_value['desc'], 'Month Rent') !== false || strpos($l_value['desc'], 'Recurrent Rent') !== false ) {
													// echo $l_value['total'];
													// echo $value_m->ID;
...
// echo "<br>";
							...
							if (strpos($lineitem['desc'], 'Rent') !== false || strpos($lineitem['desc'], 'prorated moving') !== false || strpos($lineitem['desc'], 'Month Rent') !== false || strpos($lineitem['desc'], 'Recurrent Rent') !== false ) {
							$rent_charges=(float) ($rent_charges+$lineitem['total']);
							}
							else{
														if (strpos($lineitem['desc'], 'Security Deposit') == false && strpos($lineitem['desc'], 'Security Deposit') !== 0 && strpos($lineitem['desc'], 'Invoice for') == false && strpos($lineitem['desc'], 'Standard Credit') == false  && strpos($lineitem['desc'], 'Rebill') == false ) {
...
$abc2=$abc2+$mm1;
	...
	?>
				...
				<td><?php echo $tenament_name; ?></td>
					...
					<td><a onclick="submitResourcesauxrent_multisite(<?php echo $suite_id;  ?>,<?php echo $multisuite_alldata_chunk[$suite_id]['invoice_id'];  ?>,'<?php echo $start_account; ?>','<?php echo $end_account; ?>',<?php  echo $building; ?>,'<?php echo $aux_show; ?>');" href="javascript:void(0);" title="#"><?php echo '$' . $multisuite_alldata_chunk[$suite_id]['aux']; ?></a></td>
					<!-- <td></td> -->
					</tr>
					<?php
                                   ...
                                   // echo  $value_m->ID;echo "<br>";
									...
									// echo "<br>";
										array_push($m_suite_ids_array_chunk, $m_suite_id);
													...
													// echo "<pre>";
										// array_push(array, var)
										if(array_key_exists($m_suite_id, $multisuite_alldata_chunk))
										{
										$multisuite_alldata_chunk[$m_suite_id]['rent']=$multisuite_alldata_chunk[$m_suite_id]['rent']+$l_value['total'];

										$multisuite_alldata_chunk[$m_suite_id]['invoice_id2']=$value_m->ID;
										}
										else{											
										$multisuite_alldata_chunk[$m_suite_id]['rent']=$l_value['total'];
										$multisuite_alldata_chunk[$m_suite_id]['invoice_id']=$value_m->ID;
								...
								// }
									...
									if (in_array($client_id, $y_membership_clients)) {
										# code...
													...
													if (strpos($l_value['desc'], 'Monthly Rent for') !== false || strpos($l_value['desc'], 'Month Rent') !== false || strpos($l_value['desc'], 'Recurrent Rent') !== false ) {
														// echo $l_value['total'];
														// echo $value_m->ID;
													$y_membership_items[$client_id]['rent']=$l_value['total'];
													}
				 ...
				 else
				 {
					...
					$monthlyrent=0;
				 	$suite_name_mk=get_the_title( $suite_id );
						...
						// 				if($suite_id==926)
				 	// {
						// // var_dump(strpos($lineitem['desc'], 'Security Deposit'));
				 	// // 	echo "<br>";
				 	// // 	var_dump($lineitem['desc']);
				 	// // 	echo "<br>";
				 	// 	echo "it's reaching to 926";
				 	// 	echo "lease is".$leasd_id =	$main['leases'][0];
				 	// 	echo "<pre>";
				 	// 	print_r($invoices_ids);
				 	// 	echo "</pre>";
				 	// }
...
// echo "<br>";
					...
					if (strpos($lineitem['desc'], 'Security Deposit') == false && strpos($lineitem['desc'], 'Security Deposit') !== 0 && strpos($lineitem['desc'], 'Invoice for') == false && strpos($lineitem['desc'], 'Standard Credit') == false && strpos($lineitem['desc'], 'Standard Credit') == false && strpos($lineitem['desc'], 'Rebill') == false) {
				 	...
				 	// {
				 	// 	echo "**************************************reached".$suite_id;
				 	// 	var_dump(get_post_meta( $key, '_yl_lease_id', true ));
...
// echo "<br>";
				 	...
				 	$csv_data_array[$i]['Suite Number']=get_the_title( $suite_id );
							...
							$rentboth=$multisuite_alldata_chunk[$suite_id]['rent']+$rent_charges;
							$auxboth=$multisuite_alldata_chunk[$suite_id]['aux']+$aux_charges;

								 $the_total_rent=$the_total_rent+$rentboth;
				 $array_rent[$key]=$rentboth;

				 $the_total_aux=$the_total_aux+$auxboth;
						$the_total=$the_total+$auxboth+$rentboth;
					...
					<td><?php echo get_the_title( $suite_id ); ?></td>
					<!-- <td><?php echo $tenament_name; ?></td>  -->
					<td><a onclick="submitResources(<?php echo $suite_id;  ?>,'<?php echo $start_account; ?>','<?php echo $end_account; ?>',<?php  echo $building; ?>	);" href="javascript:void(0);" title="#">$<?php echo number_format($rentboth,2); ?></a></td>
					<td><a onclick="submitResourcesauxrent(<?php echo $suite_id;  ?>,'<?php echo $start_account; ?>','<?php echo $end_account; ?>',<?php  echo $building; ?>	);" href="javascript:void(0);" title="#"><?php echo '$' . number_format($auxboth,2); ?></a></td>
					<!-- <td></td> -->
					</tr>
...
// echo "<br>";
				 ...
				 $the_total_aux=$the_total_aux+$aux_charges;
						$the_total=$the_total+$aux_charges+$rent_charges;
...
$abc2=$abc2+$mm1;
	...
	?>
					...
					<td><a onclick="submitResourcesauxrent_multisite(<?php echo $suite_id;  ?>,<?php echo $multisuite_alldata_chunk[$suite_id]['invoice_id'];  ?>,'<?php echo $start_account; ?>','<?php echo $end_account; ?>',<?php  echo $building; ?>,'<?php echo $aux_show; ?>');" href="javascript:void(0);" title="#"><?php echo '$' . number_format($multisuite_alldata_chunk[$suite_id]['aux'],2) ; ?></a></td>
					<!-- <td></td> -->
					</tr>
					<?php
...
// $resultxx = array_diff($all_invoice_id_checker22,$all_invoice_id_checker);
// print_r($resultxx);
echo "Total Amount After subtracting Rebill amount and security deposit ".$the_total;
echo "<br>";
