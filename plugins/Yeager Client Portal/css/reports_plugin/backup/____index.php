...
/********Rent Roll Report********/
	if($_POST['_accounting_report_id']=="Rent Roll Report")
		...
		<div class="mk_account_reports">
			...
			$full_total=0;
			foreach ($results as $key => $result) {
if($result->post_status=="partial" || $result->post_status=="publish" || $result->post_status=="publish" || $result->post_status=="complete")
				{
				// print_r($result);
				// exit();
			$paymentdatefull=explode(" ", $result->post_modified);		
...
$suite_array=array();
			...
			foreach ($all_data as $key => $value) {
				...
				echo $lineitem['desc']."=".$lineitem['total'];
					...
					$suite_array[$suite_name_mk]['invoice_num']=$invoice_num;
			...
			?>
				...
				// }
					...
					$csv_data_array[$i]['Suite Number']=str_replace("</p>","",$value['suite_number']);
					$csv_data_array[$i]['Tennant Name']=$value['company_name'];
					$csv_data_array[$i]['Rent']='$' . number_format($monthlyrent,2);
					$csv_data_array[$i]['Aux Charges']='$' . number_format($auxrent,2);
					// $csv_data_array[$i]['Copies']='$' . number_format($Copies);
					// $csv_data_array[$i]['Fax Line']='$' . number_format($FaxLine);
					// $csv_data_array[$i]['Fees']='$' . number_format($Fees);
					// $csv_data_array[$i]['Fax Service']='$' . number_format($FaxService);
					...
					// $csv_data_array[$i]['NSF Fee']='$' . number_format($nsf);
					// $csv_data_array[$i]['Total Other Charges']='$' . number_format($totalother);
					$csv_data_array[$i]['TOTAL']='$' . number_format($Total3);
...
$with   = array("");
		$csv_data_array2[$x]['Suite Number']=str_replace($replace, $with, $suitename);
		  // echo   $string = utf8_decode($sut['company_name']);
 // echo  $iso88591_2 = iconv('UTF-8', 'ISO-8859-1', $sut['company_name']);
					$csv_data_array2[$x]['Tennant Name']= html_entity_decode($sut['company_name'], ENT_QUOTES, 'utf-8');//iconv("UTF-8", "CP437",$sut['company_name']);
					...
					$csv_data_array2[$x]['Invoice Link']=$sut['invoice_num'];
	# code...
$x++;
}

					// $csv_data_array2[1]['xyz']='M&M Construction Services Invoice for';
			...
			//Multisuite issue
							...
							foreach ($multisuite_invoices_loop as $key_m => $value_m) {
								...
								if(get_post_meta( $value_m->ID, '_yl_lease_id', true )=="")
								{
									...
									// echo "<br>";
										...
										elseif($count==$count_lineitem)
										{
											...
											//echo "Key";
											//echo $key;
											$multisuite_alldata_chunk[$last_suite_id]['aux']=(float) ($line_item_array_chunks[$last_suite_id]['aux']+$aux_changer);
										}
										else{
										$aux_changer=$aux_changer+$l_value['total'];
										}
								...
								if($yl_suite_number == -1)
								{
									...
									$y_membership_items[$client_id]['invoice_id']=0;
				...
				<?php
				 ...
				 else
				 {
					...
					$monthlyrent=0;
				 	...
				 	// echo $suite_id;
				 	if(array_key_exists($suite_id, $multisuite_alldata))
				 	{
				 		if($multisuite_alldata[$suite_id]['aux']!=0)
				 		{
				 			$aux_show="show";
				 		}
				 		else{
					...
					<td><a onclick="submitResources_multisuite(<?php echo $suite_id;  ?>,<?php echo $multisuite_alldata_chunk[$suite_id]['invoice_id'];  ?>,'<?php echo $start_account; ?>','<?php echo $end_account; ?>',<?php  echo $building; ?>	);" href="javascript:void(0);" title="#">$<?php echo $multisuite_alldata_chunk[$suite_id]['rent']; ?></a></td>
...
// 						    // )
// 								);	
								echo		$start_Account2=strtotime($start_account);
								echo	$end_Account2=strtotime($end_account);
								$args = array(
							    'post_type' => 'sa_invoice',
							    'post_status' => 'any',
							    'posts_per_page' => -1,
